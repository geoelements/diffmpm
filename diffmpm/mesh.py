...
class Mesh2D:
    ...
    def _get_element_node_ids(self, element_idx):
        """
        Return nodal ids of element `element_idx`.
        ...
        ---------
        element_idx: int
            Element ID to return the nodal ids for.

        Returns
        -------
        array_like
            Nodal ids in the order as explained in the figure.
        """
        lower_left = (element_idx // self.nelements[0]) * (
            self.nelements[0] + 1
        ) + (element_idx % self.nelements[0])
        return jnp.asarray(
            [
                lower_left,
                lower_left + 1,
    ...
    def _get_element_node_pos(self, element_idx):
        ...
        mapping node coordinates for that element.
        """
        node_ids = self._get_element_node_ids(element_idx)
        return self.nodes.position[node_ids]
